...
def chart_svg(cx, w, h):
    ...
    for kind in ('c:barChart', 'c:lineChart'):
        for blk in d.getElementsByTagName(kind):
            for s in ch(blk, 'c:ser'):
                ...
                sers.append((kind, name, col, vals, dash))
    ...
    bw = pw / n * 0.6 / nb
    for j, (_, name, col, vals, _) in enumerate(bars):
        for i, v in enumerate(vals):
            if v is None:
                continue
            x = X(i) - pw / n * 0.3 + (0 if stackish else j) * bw
            y0, y1 = Y(max(lo, 0)), Y(v)
            svg.append('<rect x="%.1f" y="%.1f" width="%.1f" height="%.1f" fill="%s"/>' % (x, min(y0, y1), bw, abs(y1 - y0), col))
    for kind, name, col, vals, dash in sers:
        if kind != 'c:lineChart':
            continue
