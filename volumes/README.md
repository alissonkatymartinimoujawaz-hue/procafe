...
**Excel dashboard**: `suivi_cecafe.xlsx`, regenerated by `build_xlsx.py` from
`cecafe_daily.csv` (the workflow does it after each fetch). Sheets: Definitions
(how to read the page), Donnees (raw rows, paste new days here), Arabica and
Robusta (a "Date affichée" cell drives everything: day D in red, month-to-date
